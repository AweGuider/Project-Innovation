...
public class CoinSpawner : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < Coin.maxAmount; i++)
        {
            while (true)
            {
                Vector3 tempPos = new Vector3(Random.Range(-1.5f, 55), coinPrefab.transform.position.y, Random.Range(-27, 9.5f));

                GameObject coin = Instantiate(coinPrefab, tempPos, Quaternion.identity);
                coins.Add(coin);
